/*
  Warnings:

  - You are about to drop the column `permissioName` on the `permissions` table. All the data in the column will be lost.
  - Added the required column `permissionName` to the `Permissions` table without a default value. This is not possible if the table is not empty.

*/
-- AlterTable
ALTER TABLE `permissions` DROP COLUMN `permissioName`,
    ADD COLUMN `permissionName` VARCHAR(191) NOT NULL;
